const CALENDAR_NUMBER_CHANGE_ANIMATION_TIME = 300;
const MODAL_ANIMATION_TIME = 500;
const INPUT_DROPDOWN_ANIMATION_TIME = 300;
const HEADER_TEXT = '우아한 가계부';
const PAYMENT_MODAL_TITLE = '추가하실 결제수단을 적어주세요.';
const PAYMENT_MODAL_CANCEL_TEXT = '취소';
const PAYMENT_MODAL_SUBMIT_TEXT = '등록';
const PAYMENT_MODAL_PLACEHOLDER = '입력하세요';
const PIE_CHART_TRANSITION_WAIT = 300;
const PIE_CHART_TRANSITION_DURATION = 2000;
const PIE_CHART_SIZE = 400;
const ALERT_OUT_TIME = 3500;
const SECOND_PER_FRAME = 50;

//  event constant
const SET_HISTORY_DATA = 'SET_HISTORY_DATA';
const SET_USER_DATA = 'SET_USER_DATA';
const PAYMENT_ADD_EVENT = 'PAYMENT_ADD_EVENT';
const PAYMENT_DEL_EVENT = 'PAYMENT_DEL_EVENT';
const HISTORY_ADD_EVENT = 'HISTORY_ADD_EVENT';
const SET_USER_META_DATA = 'SET_USER_META_DATA';
const HISTORY_MODIFY_EVENT = 'HISTORY_MODIFY_EVENT';
const HISTORY_DELETE_EVENT = 'HISTORY_DELETE_EVENT';
const LOGIN_MODAL_TITLE =
  '우아한 가계부를 사용하기 위해선 로그인이 필요합니다.';
const LOGIN_MODAL_CANCEL_TEXT = '데모 계정으로 진행';
const OAUTH_CODE_SEP = '?code=';
const DATE_UP_EVENT = 'DATE_UP_EVENT';
const DATE_DOWN_EVENT = 'DATE_DOWN_EVENT';
const GET_HISTORIES_BY_DATE = 'GET_HISTORIES_BY_DATE';
const GET_LINE_CHART_DATA = 'GET_LINE_CHART_DATA';
const CLEAN_UP_DATA = 'CLEAN_UP_DATA';

const COLOR_CONSTANT = {
  CATEGORY_1: '#6ED5EB',
  CATEGORY_2: '#4CB8B8',
  CATEGORY_3: '#94D3CC',
  CATEGORY_4: '#4CA1DE',
  CATEGORY_5: '#D092E2',
  CATEGORY_6: '#817DCE',
  CATEGORY_7: '#4A6CC3',
  CATEGORY_8: '#B9D58C',
  CATEGORY_9: '#E6D267',
  CATEGORY_10: '#E2B765',
};

const LINE_CHART = {
  CURVE_RATIO: 0.4,
  MIN_CURVE_VALUE: 40,
  MAX_VALUE_RATIO: 1.2,
  POINT_WAIT_TIME: 0.9,
  POINT_WAIT_INTERVAL: 0.2,
  WIDTH: 720,
  HEIGHT: 330,
  INTERVAL: 30,
};

export {
  CALENDAR_NUMBER_CHANGE_ANIMATION_TIME,
  INPUT_DROPDOWN_ANIMATION_TIME,
  MODAL_ANIMATION_TIME,
  HEADER_TEXT,
  PAYMENT_MODAL_TITLE,
  PAYMENT_MODAL_CANCEL_TEXT,
  PAYMENT_MODAL_SUBMIT_TEXT,
  PAYMENT_MODAL_PLACEHOLDER,
  COLOR_CONSTANT,
  PIE_CHART_TRANSITION_WAIT,
  PIE_CHART_TRANSITION_DURATION,
  SECOND_PER_FRAME,
  ALERT_OUT_TIME,
  LINE_CHART,
  SET_HISTORY_DATA,
  SET_USER_DATA,
  SET_USER_META_DATA,
  PAYMENT_ADD_EVENT,
  PAYMENT_DEL_EVENT,
  HISTORY_ADD_EVENT,
  HISTORY_MODIFY_EVENT,
  HISTORY_DELETE_EVENT,
  LOGIN_MODAL_TITLE,
  LOGIN_MODAL_CANCEL_TEXT,
  OAUTH_CODE_SEP,
  DATE_UP_EVENT,
  DATE_DOWN_EVENT,
  GET_HISTORIES_BY_DATE,
  GET_LINE_CHART_DATA,
  PIE_CHART_SIZE,
  CLEAN_UP_DATA,
};
